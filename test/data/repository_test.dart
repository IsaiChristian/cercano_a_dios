...
void main() {
  ...
  PrayerSession session({String? audio}) => PrayerSession(
    id: 'one',
    promptId: 'p01',
    promptText: 'Thank you, Lord.',
  ...
  Future<List<PrayerSession>> read() async => (await repository.sessions())
      .fold((f) => throw StateError(f.message), (v) => v);
  ...
  test('malformed data reaches left boundary', () async {
    await db.insert('sessions', {
      'id': 'bad',
      'promptId': 'test',
      'promptText': 'test',
      'localDate': '2023',
      'completedAt': 'bad-date', // This will cause DateTime.parse to throw
      'duration': 10,
      'offsetMinutes': 10,
      'spoken': 1,
      'audioPath': null,
    });
    final res = await repository.sessions();
    expect(res.isLeft(), true);
  });
  test(
    'audio deletion keeps a moment and streak, session deletion removes both',
    () async {
      ...
      expect(
        calculateProgress(await read(), DateTime(2026, 9, 15)).currentStreak,
        1,
      );
  ...
  test('missing audio is not committed as a successful recording', () async {
    expect(
      (await repository.complete(session(audio: 'missing.m4a'))).isLeft(),
      true,
    );
    expect(await read(), isEmpty);
  });
  test(
    'startup removes orphaned recordings but preserves saved ones',
    () async {
  ...
  test('reset deletes recordings, reminders, and history', () async {
    ...
    await repository.saveReminder(
      const Reminder(id: 1, hour: 7, minute: 30, weekdays: [1, 3, 5]),
    );
